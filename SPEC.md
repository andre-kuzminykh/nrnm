...
| FR-39 | Память организована как **дерево файлов**: папки произвольной вложенности + файлы на любом уровне. Пользователь может создавать/переименовывать/удалять папки и перемещать файлы |
| FR-40 | Нажатие «📁 Память» без выбора конкретной папки запускает **RAG по ВСЕМ файлам** пользователя. Заход внутрь папки ограничивает RAG **её содержимым** (рекурсивно). Выбор конкретного файла — RAG/чат **только по нему** |
| FR-41 | Внутри любой папки или файла пользователь может **общаться** (задавать вопросы) — контекст автоматически ограничен тем уровнем дерева, где он находится |
| FR-42 | На каждом уровне дерева есть кнопка **«📁 Выбрать все»** — выделяет все файлы текущей папки (и подпапок) как scope для RAG |
| FR-43 | Внутри папки можно **создать подпапку** (➕) или **загрузить файл** (📎 — отправить файл в чат). Файлы попадают в текущую открытую папку |
