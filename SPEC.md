...
| FR-39 | Память = **дерево файлов**: папки произвольной вложенности + файлы. Файлы в папке показываются как **гиперссылки в тексте** сообщения, а не как inline-кнопки |
| FR-40 | RAG scope автоматически привязан к уровню навигации: корень = все файлы, папка = рекурсивно, файл = только он. Текст в папке = вопрос по scope |
| FR-41 | На форме файла (tap по гиперссылке): карточка файла + чат в контексте **только этого файла** + удалить + назад |
| FR-42 | **Пагинация** файлов: если в папке >10 файлов, появляются кнопки ← → для листания. При ≤10 — кнопок нет |
| FR-43 | Внутри папки: **➕ Папка** (создать подпапку) + загрузка файлов (📎 отправить в чат). В ответах RAG файлы-источники — **гиперссылки** на форму файла |
| FR-44 | **Все инструменты** (чат, память, веб) поддерживают контекст диалога: follow-up вопросы учитывают предыдущие ответы. Веб-поиск формирует поисковый запрос с учётом истории, а затем LLM синтезирует ответ на основе результатов + истории |
| FR-45 | Системные промпты хранятся в **отдельных .md файлах** (`prompts/system/`), загружаются через `services/prompt_loader.py`. Можно редактировать без перезапуска бота (перечитываются при каждом вызове) |
| FR-46 | Файлы в списке папки отображаются как `[filename.pdf]` — кликабельная ссылка (callback на форму файла). Квадратные скобки включены чтобы пользователь мог скопировать и вставить в текст для подключения `[контекст]` |
| FR-47 | Суперагент может **спросить пользователя** mid-run (`ask_user` tool): выполнение паузится, бот задаёт вопрос, ждёт ответ, передаёт его в граф и возобновляет. Также может спросить на этапе планирования |
| FR-48 | Суперагент может **искать по внутренней базе** (`rag_search` tool): выполняет RAG-запрос по выбранным доменам пользователя и возвращает top-k результаты с фрагментами |
| FR-49 | Суперагент может **открыть файл** из дерева файлов (`file_open` tool): загружает полный контент файла по имени или пути. Если файл не влезает в контекст (`max_chars`), автоматически суммаризирует через LLM (map-reduce) |
| FR-50 | Все инструменты суперагента (`web_search`, `rag_search`, `file_open`, `ask_user`) автоматически доступны LLM-планировщику с описаниями; планировщик сам решает какие использовать |
| FR-51 | Суперагент **помнит контекст** между задачами: если пользователь спросил "рынок роботов на Кипре", а потом "А в Греции?" — планировщик видит предыдущий диалог и знает что речь о роботах |
| FR-52 | Прогресс-бар показывает **все параллельные шаги** одновременно, каждый со своим статусом (tool call, critic, alignment) — полная прозрачность |
...
| FR-44  | `tests/test_us9_context_prompts.py`   |
| FR-45  | `tests/test_us9_context_prompts.py`   |
| FR-47  | `tests/test_us10_agent_tools.py`      |
| FR-48  | `tests/test_us10_agent_tools.py`      |
| FR-49  | `tests/test_us10_agent_tools.py`      |
| FR-50  | `tests/test_us10_agent_tools.py`      |
| FR-39  | `tests/test_us8_file_tree.py`        |
| FR-40  | `tests/test_us8_file_tree.py`        |
| FR-41  | `tests/test_us8_file_tree.py`        |
| FR-42  | `tests/test_us8_file_tree.py`        |
| FR-43  | `tests/test_us8_file_tree.py`        |
